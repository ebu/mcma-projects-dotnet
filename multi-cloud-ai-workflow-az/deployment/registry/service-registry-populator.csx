...
public class ServiceRegistryPopulator
{
    ...
    private static readonly JObject ServicesJson = JObject.Parse(File.ReadAllText("./deployment/registry/services.json"));

    private static readonly JArray JobProfilesJson = JArray.Parse(File.ReadAllText("./deployment/registry/profiles.json"));
    ...
    public ResourceManager GetResourceManager(IResourceManagerProvider resourceManagerProvider)
        => resourceManagerProvider.Get(TerraformOutput.ServicesUrl, ServiceRegistry.AuthType, ServiceRegistry.AuthContext);
}