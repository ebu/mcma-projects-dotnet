...
resource "azurerm_function_app" "ame_service_worker_function" {
  ...
  provisioner "local-exec" {
    command = "az webapp start --resource-group ${var.resource_group_name} --name ${azurerm_function_app.ame_service_worker_function.name}"
  }
}
...
output ame_service_worker_url {
  value = "https://${azurerm_function_app.ame_service_worker_function.default_hostname}/"
}
