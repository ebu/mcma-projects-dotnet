locals {
  api_zip_file = "./../services/Mcma.Azure.JobRepository/ApiHandler/dist/function.zip"
}

resource "azurerm_storage_blob" "job_repository_api_function_zip" {
  name                   = "job-repository/function_${filesha256("${local.zip_file}")}.zip"
...
data "azurerm_storage_account_sas" "job_repository_api_function_sas" {
...
resource "azurerm_function_app" "job_repository_api_function" {
  ...
  app_settings = {
    ...
    WEBSITE_RUN_FROM_PACKAGE       = "https://${var.app_storage_account_name}.blob.core.windows.net/${var.deploy_container}/${azurerm_storage_blob.job_repository_api_function_zip.name}${data.azurerm_storage_account_sas.job_repository_api_function_sas.sas}"
...
resource "azurerm_template_deployment" "job_repository_function_key" {
  name                = "jobrepositoryfunckeys"
  resource_group_name = "${var.resource_group_name}"
...
output "job_repository_key" {
  value = "${lookup(azurerm_template_deployment.job_repository_function_key.outputs, "functionkey")}"
}
