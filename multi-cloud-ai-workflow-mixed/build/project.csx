...
public abstract class BuildProject : BuildTask
{
    private const string DistFolder = "dist";
    private const string StagingFolder = DistFolder + "/staging";

    protected BuildProject(string projectFolder, bool build = true, bool clean = true)
    {
        ...
        if (build)
        {
            ...
            CheckForOutputChanges = new CheckForFileChanges(StagingFullPath, $"{DistFullPath}/function.zip", StagingFolderChangeCheckExcludes);
        ...
        Zip = new ZipTask(
            StagingFullPath,
            $"{DistFullPath}/function.zip",
            externalAttributes: ZipFileExternalAttributes);
    }
    ...
    protected virtual string[] StagingFolderChangeCheckExcludes { get; } = new string[0];
...
public class BuildAwsLambdaFunctionProject : BuildProject
{
    public BuildAwsLambdaFunctionProject(string projectFolder, bool build = true, bool clean = true)
        : base(projectFolder, build, clean)
    {
    }

    protected override string[] StagingFolderChangeCheckExcludes { get; } = new[] {"\\.json"};

    protected override IDictionary<string, int> ZipFileExternalAttributes =>
...
public class BuildAzureFunctionProject : BuildProject
{
    public BuildAzureFunctionProject(string projectFolder, bool build = true, bool clean = true)
        : base(projectFolder, build, clean)
    {
    }
}