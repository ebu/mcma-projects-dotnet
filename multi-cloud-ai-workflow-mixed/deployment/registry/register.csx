...
public class UpdateServiceRegistry : BuildTask
{
    ...
    protected override async Task<bool> ExecuteTask()
    {
        ...
        var SERVICES = CreateServices(terraformOutput, serviceUrlsAndKeys);
    ...
    private static IDictionary<string, Service> CreateServices(IDictionary<string, string> serviceUrls, IDictionary<string, (string, string)> serviceUrlsAndKeys)
    {
            ...
            switch (prop)
            {
                ...
                case "media_repository_url":
                    serviceList.Add(new Service
                    {
                        ...
                        Resources = new[]
                        {
                            new ResourceEndpoint {ResourceType = "BMContent", HttpEndpoint = serviceUrls[prop].TrimEnd('/') + "/bm-contents"},
                            new ResourceEndpoint {ResourceType = "BMEssence", HttpEndpoint = serviceUrls[prop].TrimEnd('/') + "/bm-essences"}
                        },
                        AuthType = "AzureFunctionKey",
                        AuthContext =
                            new AzureFunctionKeyAuthContext(EncryptionHelper.Encrypt(serviceUrlsAndKeys["media_repository_url"].Item2, PublicEncryptionKeyJson))
                                .ToMcmaJson()
                                .ToString()
                    });
                    break;
