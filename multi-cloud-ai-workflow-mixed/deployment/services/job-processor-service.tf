...
resource "aws_lambda_function" "job-processor-service-api-handler" {
  filename         = "./../services/Mcma.Aws.JobProcessor/ApiHandler/dist/function.zip"
  ...
  source_code_hash = "${filebase64sha256("./../services/Mcma.Aws.JobProcessor/ApiHandler/dist/function.zip")}"
...
resource "aws_lambda_function" "job-processor-service-worker" {
  filename         = "./../services/Mcma.Aws.JobProcessor/Worker/dist/function.zip"
  ...
  source_code_hash = "${filebase64sha256("./../services/Mcma.Aws.JobProcessor/Worker/dist/function.zip")}"
  runtime          = "dotnetcore2.1"
  timeout          = "30"
  memory_size      = "256"
}
